...
if sys.platform != "win32":
    ...
    def apply_memory_limit(memory_limit_mb):
        ...
        if not isinstance(memory_limit_mb, (int, float)) or memory_limit_mb <= 0:
            warnings.warn(
                f"Invalid memory limit specified: {memory_limit_mb}. Must be a positive number."
            )
            return
        ...
        if _assign_job(job):
            ...
            print(
                f"Successfully applied memory limit of {memory_limit_mb} MB to the current process."
            )
...
if __name__ == "__main__":
    ...
    if sys.platform == "win32":
        ...
        try:
            ...
            print(
                "Memory allocation succeeded (this should not happen if the limit is effective)."
            )
        except MemoryError:
            print(
                "Successfully caught MemoryError. The memory limit is working as expected."
            )
