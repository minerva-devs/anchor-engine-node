...
class DistillerClient:
    ...
    def __init__(
        self, base_url: str = os.getenv("DISTILLER_URL", "http://localhost:8001")
    ):
...
class QLearningAgentClient:
    ...
    def __init__(
        self, base_url: str = os.getenv("QLEARNING_URL", "http://localhost:8002")
    ):
        self.base_url = base_url
        self.client = httpx.AsyncClient()
    ...
    async def find_optimal_path(
        ...
        try:
            ...
            response = await self.client.post(
                f"{self.base_url}/find_optimal_path", json=request_data.dict()
            )
    ...
    async def refine_relationships(
        self, path: MemoryPath, reward: float
    ) -> Dict[str, Any]:
        """
        ...
        """
        try:
            request_data = {"path": path.dict(), "reward": reward}

            response = await self.client.post(
                f"{self.base_url}/refine_relationships", json=request_data
            )
            ...
            if response.status_code == 200:
                ...
                logger.error(f"QLearningAgent returned status {response.status_code}")
                return {
                    "error": f"QLearningAgent returned status {response.status_code}"
                }
        except Exception as e:
            logger.error(f"Error calling QLearningAgent: {str(e)}")
...
class InjectorClient:
    ...
    def __init__(
        self, base_url: str = os.getenv("INJECTOR_URL", "http://localhost:8004")
    ):
        self.base_url = base_url
        self.client = httpx.AsyncClient()
        ...
        try:
            ...
            poml_request = POML(
                ...
                directive={
                    ...
                    "task": {"name": "InjectData", "data": sanitized_data},
                },
            )
            ...
            if callable(error_str):
                ...
                return {"success": False, "error": "Unexpected callable error string"}
        ...
        try:
            ...
            poml_request = POML(
                ...
                directive={
                    ...
                    "task": {"name": "CreateTimeNode", "timestamp": dt.isoformat()},
                },
            )
...
class ArchivistClient:
    ...
    def __init__(
        self, base_url: str = os.getenv("ARCHIVIST_URL", "http://localhost:8003")
    ):
        self.base_url = base_url
        self.client = httpx.AsyncClient()
