...
class TestArchivistIntegration(unittest.TestCase):
    ...
    def test_context_request(self):
        ...
        context_request = ContextRequest(
            query="Test query for context", user_id="test_user"
        )
    ...
    def test_distiller_data_endpoint(self):
        ...
        with (
            patch(
                "ece.agents.tier3.archivist.archivist_agent.injector_client"
            ) as mock_injector_client,
            patch(
                "ece.agents.tier3.archivist.archivist_agent.qlearning_client"
            ) as mock_qlearning_client,
            patch(
                "ece.agents.tier3.archivist.archivist_agent.distiller_client"
            ) as mock_distiller_client,
        ):
            # Mock the send_data_for_injection method to return a success response
            mock_injector_client.send_data_for_injection = AsyncMock(
                return_value={
                    "success": True,
                    "status": "processed",
                    "memory_node_id": 123,
            ...
            mock_qlearning_client.refine_relationships = AsyncMock(
                return_value={"status": "success"}
            )
            # Mock the process_text method
            mock_distiller_client.process_text = AsyncMock(
                return_value={
                    "entities": [
                        {
                            "id": "test_entity_1",
            ...
            mock_injector_client.get_or_create_timenode = AsyncMock(
    ...
    def test_handle_truncated_entries(self):
        ...
        with (
            patch(
                "ece.agents.tier3.archivist.archivist_agent.redis_client"
            ...
            response = self.client.post(
                "/internal/handle_truncated_entries", json=truncated_keys
            )
            ...
            self.assertEqual(
                mock_qlearning_client.refine_relationships.call_count,
                len(truncated_keys),
            )
