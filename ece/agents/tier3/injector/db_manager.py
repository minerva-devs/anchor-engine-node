        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                if self._is_transient_error(e) and attempt < self.max_retries:
                    ...
                    logger.warning(
                        f"Transient error occurred: {e}. Retrying in {wait_time:.2f} seconds (attempt {attempt + 1}/{self.max_retries})"
                    )
                    time.sleep(wait_time)
                else:
                    logger.error(
                        f"Failed to execute query after {attempt} attempts: {e}"
                    )
        ...
        for attempt in range(self.max_retries + 1):
            try:
                with self.driver.session() as session:
                    ...
                    try:
                        ...
                        for i, query_dict in enumerate(queries):
                            ...
                            logger.debug(
                                f"Executing query {i + 1}/{len(queries)}: {query[:100]}..."
                            )  # Log first 100 chars
                            tx.run(query, parameters)
                        tx.commit()
                        logger.info(
                            f"Transaction committed successfully with {len(queries)} queries"
                        )
                        return True
                    except Exception as e:
                        tx.rollback()
                ...
                if self._is_transient_error(e) and attempt < self.max_retries:
                    ...
                    logger.error(
                        f"Failed to execute transaction after {attempt} attempts: {e}"
                    )
                    raise
        ...
        if "entities" in data:
            ...
            for entity in data["entities"]:
                ...
                if entity_id is not None:
                    ...
                    queries.append({"query": query.strip(), "parameters": parameters})
                else:
        ...
        if "relationships" in data:
            ...
            for relationship in data["relationships"]:
                ...
                if start_id is None or end_id is None:
                    logger.error(
                        f"Relationship has null IDs after UUID generation: start_id={start_id}, end_id={end_id}"
                    )
                ...
                logger.debug(
                    f"Start ID type: {type(start_id)}, End ID type: {type(end_id)}"
                )

                # Check if IDs are still null
                if start_id is None or end_id is None:
                    logger.error(
                        f"Relationship has null IDs: start_id={start_id}, end_id={end_id}"
                    )
                    continue
