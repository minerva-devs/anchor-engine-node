...
class InjectorAgent:
    ...
    def receive_data_for_injection(self, data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        """
        logger.info(
            f"InjectorAgent received data for injection. Entities: {len(data.get('entities', []))}, Relationships: {len(data.get('relationships', []))}"
        )
        try:
            # Translate the incoming data into Cypher queries
            cypher_queries = self.neo4j_manager._translate_to_cypher(
                data
            ...
            return {
                "success": False,
                "message": f"Error during data injection: {str(e)}",
            }
    ...
    def get_or_create_timenode(self, timestamp_str: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "success": True,
                "message": "Time node retrieved or created.",
                "time_node": result,
            }
        except ValueError:
            ...
            return {
                "success": False,
                "message": "Invalid timestamp format. Please use ISO format.",
            }
        except Exception as e:
            logger.error(f"Error getting or creating time node: {e}")
            return {
                "success": False,
                "message": f"Error getting or creating time node: {str(e)}",
            }

    def link_memory_to_timenode(
        self, memory_node_id: int, timestamp_str: str
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.neo4j_manager.link_memory_to_timenode(memory_node_id, timestamp):
                logger.info(
                    f"Memory node {memory_node_id} linked to time node for {timestamp_str}."
                )
                return {
                    "success": True,
                ...
                logger.error(
                    f"Failed to link memory node {memory_node_id} to time node for {timestamp_str}."
                )
                return {
                    "success": False,
                    "message": "Failed to link memory node to time node.",
                }
        except ValueError:
            logger.error(f"Invalid timestamp format: {timestamp_str}")
            return {
            ...
            return {
                "success": False,
                "message": f"Error linking memory node to time node: {str(e)}",
            }
