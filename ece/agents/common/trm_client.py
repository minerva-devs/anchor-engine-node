...
class TRMClient:
    ...
    async def health_check(self) -> bool:
        ...
        try:
            ...
            response = await self.client.get(f"{self.config.api_base.replace('/v1', '')}/v1/models")
            return response.status_code == 200
        except Exception as e:
            self.logger.error(f"TRM health check failed: {e}")
            return False
    
    async def refine_thought(self, query: str, current_thought: str, carryover: str = "") -> str:
        """
        ...
        try:
            ...
            if "choices" in result and len(result["choices"]) > 0:
                ...
                try:
                    ...
                    critique_data = eval(critique_text)  # Note: In production, use json.loads instead of eval
                    # Validate the structure
                    if "valid" in critique_data and "issues" in critique_data and "suggestions" in critique_data:
                        return critique_data
                except:
                    self.logger.warning(f"Could not parse critique JSON: {critique_text}")
    ...
    async def iterative_refinement(self, query: str, initial_draft: str, 
        ...
        for iteration in range(max_iterations):
            ...
            if previous_thought:
                ...
                similarity = self._calculate_similarity(previous_thought, current_thought)
                if 1 - similarity < self.config.stability_threshold:
                    self.logger.info(f"TRM refinement stabilized at iteration {iteration + 1}")
                    break
            
            # Refine the thought based on critique
            refined_thought = await self.refine_thought(query, current_thought, carryover)
    ...
    async def process_chunk(self, query: str, context_chunk: str, carryover: str = "") -> str:
        """
