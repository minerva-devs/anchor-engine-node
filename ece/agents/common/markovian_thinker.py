...
class MarkovianThinker:
    ...
    async def markovian_reasoning_loop(self, initial_query: str, context: str = "") -> str:
        """
        ...
        for iteration in range(self.config.iteration_cap):
            ...
            max_new_tokens = (
                ...
                if iteration == 0 
                else self.config.thinking_context_size - self.config.markovian_state_size
            )
            
            try:
                logger.info(f"Processing chunk {iteration + 1}/{self.config.iteration_cap}")
                ...
                if self.config.eos_token_id and self.config.eos_token_id in response_text:
        ...
        logger.info(f"Markovian reasoning completed, total tokens: {len(final_response)}")
    ...
    async def _generate_chunk(self, prompt: str, max_new_tokens: int) -> str:
        ...
        messages = [
            {"role": "system", "content": "You are a helpful reasoning assistant. Think step by step in a structured way. Be thorough but concise in each chunk."},
        ...
        for attempt in range(self.config.max_retries):
            try:
                ...
                if attempt == self.config.max_retries - 1:
                    raise Exception(f"Failed to generate chunk after {self.config.max_retries} attempts: {str(e)}")
...
class ReasoningAnalyzer:
    ...
    def should_use_markovian_thinking(prompt: str) -> bool:
        ...
        complexity_indicators = [
            'analyze', 'evaluate', 'compare', 'contrast', 'examine', 
            'investigate', 'research', 'deep', 'thorough', 'comprehensive',
            'strategy', 'plan', 'solution', 'approach', 'methodology',
            'explain in detail', 'step by step', 'break down', 'elaborate',
            'reason through', 'think through', 'consider', 'assess',
            'solve', 'derive', 'calculate', 'prove', 'demonstrate',
            'why', 'how', 'what is the process', 'outline', 'describe in depth'
        ]
        
        prompt_lower = prompt.lower()
        for indicator in complexity_indicators:
            if indicator in prompt_lower:
                logger.info(f"Using Markovian thinking for prompt containing '{indicator}'")
                return True
