 ...
 */
export async function recordDistill(metadata) {
    ...
    try {
        await db.run(`INSERT INTO distills (
 ...
 * Get a distill by ID
 */
export async function getDistill(id) {
    try {
 ...
 */
export async function getDistillsBySession(sessionId) {
    try {
        const result = await db.run('SELECT * FROM distills WHERE $1 = ANY(source_sessions) ORDER BY timestamp DESC', [sessionId]);
...
export async function getAllDistills(limit = 50) {
    try {
        const result = await db.run('SELECT * FROM distills ORDER BY timestamp DESC LIMIT $1', [limit]);
        return (result.rows || []).map((row) => ({
            id: row.id,
            timestamp: row.timestamp,
            filename: row.filename,
 ...
 */
export async function deleteDistill(id) {
    try {
