...
export async function getBrightNodes(query, buckets = [], maxNodes = config.SEARCH.max_chars_limit) {
    ...
    const brightNodes = topResults.map(result => ({
        id: result.id,
    ...
    for (let i = 0; i < brightNodes.length; i++) {
        ...
        for (let j = 0; j < brightNodes.length; j++) {
            ...
            const sharedBuckets = currentNode.buckets.filter((b) => otherNode.buckets.includes(b));
            const sharedTags = currentNode.tags.filter((t) => otherNode.tags.includes(t));
 ...
 */
export async function getStructuredGraph(query, buckets = []) {
    ...
    return {
        ...
        edges: brightNodes.flatMap(node => node.relationships.map(rel => ({
            source: node.id,
            target: rel.targetId,
