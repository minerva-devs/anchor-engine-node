 ...
 */
export function stripTagFooters(content) {
    if (!content)
        return '';
 ...
 */
export function getItems(input) {
 ...
 */
export async function coalesceByProximity(results, proximityThreshold = 500, maxSnippets = 500) {
    // Group by compound_id (source file)
    const byCompound = new Map();
    results.forEach(r => {
        if (!r.compound_id || !r.source)
            return;
        if (!byCompound.has(r.compound_id))
            byCompound.set(r.compound_id, []);
...
async function inflateSnippetFromDisk(snippet) {
    try {
 ...
 */
function snapToSentenceBoundaries(content, targetStart, targetEnd) {
 ...
 */
export async function formatResults(results, maxChars, options) {
    try {
...
${s.content}
        ...
        const enrichedResults = deduplicatedSnippets.map(s => ({
        ...
        return {
            ...
            toAgentString: () => {
                return deduplicatedSnippets.map(s => `[${s.provenance}] ${s.source} (t=${s.temporal_weight.toFixed(3)}, atoms=${s.sourceAtoms.length}): ${s.content.substring(0, 200)}...`).join('\n');
            },
            metadata: {
...
export function filterDisplayTags(tags) {
    if (!config.SEARCH?.hide_years_in_tags)
        return tags;
