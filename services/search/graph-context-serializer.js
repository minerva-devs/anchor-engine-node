...
export function detectIntent(query) {
    ...
    EMOTIONAL_SIGNALS.forEach(s => { if (q.includes(s))
        scores.emotional += 2; });
    TEMPORAL_SIGNALS.forEach(s => { if (q.includes(s))
        scores.temporal += 2; });
    RELATIONAL_SIGNALS.forEach(s => { if (q.includes(s))
        scores.relational += 1; });
    CREATIVE_SIGNALS.forEach(s => { if (q.includes(s))
        scores.creative += 2; });
    // Default to factual
    scores.factual = 1;
 ...
 */
function toMemoryNode(result, physics) {
    return {
        id: result.id,
 ...
 */
function anchorsToMemoryNodes(anchors) {
    if (anchors.length === 0)
        return [];
 ...
 */
function walkerToMemoryNodes(walkerResults) {
    return walkerResults.map(pr => toMemoryNode(pr.result, pr.physics));
}
// =============================================================================
// TIME FORMATTING
// =============================================================================
function formatTimeDrift(deltaMs) {
    const hours = deltaMs / (1000 * 60 * 60);
    if (hours < 1)
        return `${Math.round(deltaMs / (1000 * 60))}m ago`;
    if (hours < 24)
 ...
 */
function hoistCommonTags(nodes) {
    if (nodes.length === 0)
        return [];
    const freq = new Map();
    for (const node of nodes) {
 ...
 */
function uniqueNodeTags(node, taxonomySet) {
    return (node.tags || [])
 ...
 */
function inferNodeLocation(node) {
    if (node.id.startsWith('virtual_mem_'))
        return '[state-anchor]';
 ...
 */
export function serializeForLLM(pkg, charBudget) {
    const budget = charBudget || Infinity;
    let output = '';
    let currentChars = 0;
    const add = (s) => { output += s; currentChars += s.length; };
 ...
 */
export function assembleAndSerialize(opts) {
