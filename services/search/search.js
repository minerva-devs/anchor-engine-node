...
import { searchResultCache as lruSearchCache } from '../../utils/lru-cache.js';
// --- Clean Mode Artifact Patterns ---
import { stripArtifacts, validateCleanContent } from '../../types/search.js';
...
function getCacheKey(query, buckets, maxChars, tags, provenance, useMaxRecall) {
...
export async function findAnchors(query, buckets = [], tags = [], _maxChars = config.SEARCH.max_chars_default, provenance = 'all', filters, fuzzy = false) {
    ...
    `;
        ...
        try {
            ...
            const contentBuckets = new Map(); // bucket key -> bucket
            // Get bucket key based on content length and fingerprint prefix
            const getBucketKey = (normalizedContent) => {
            ...
            const getAdjacentBuckets = (length) => {
            ...
            for (const candidate of anchors) {
                ...
                for (const adjBucketNum of adjacentBuckets) {
                    ...
                    for (const [key, adjBucket] of contentBuckets.entries()) {
                        if (!key.startsWith(`${adjBucketNum}:`))
                            continue;
 ...
 */
export async function executeMoleculeSearch(query, bucket, buckets, maxChars = config.SEARCH.max_chars_default, deep = false, provenance = 'all', explicitTags = [], userContext, mode = 'standard') {
 ...
 * @param useMaxRecall - If true, uses MAX_RECALL_CONFIG for comprehensive retrieval
 */
export async function iterativeSearch(query, buckets = [], maxChars = config.SEARCH.max_chars_default, tags = [], provenance = 'all', useMaxRecall = false, userContext, mode = 'standard') {
    // Memory-aware throttling
    const throttleResult = await throttleSearchForMemory();
    if (!throttleResult.proceed) {
        throw new Error(`Search rejected: ${throttleResult.reason}. Please wait and try again.`);
 ...
 */
async function handlePrefixQuery(query, buckets = [], maxChars = 20000, tags = []) {
    ...
    if (prefix.startsWith('distill:')) {
        ...
        if (!bucketParam) {
            ...
            try {
                const distills = await db.run('SELECT id, timestamp, filename, file_path, line_count, lines_unique, compression_ratio, source_sessions, source_files, parameters FROM distills ORDER BY timestamp DESC LIMIT 50');
                return {
            ...
            try {
                ...
                const distills = await db.run(`SELECT id, timestamp, filename, file_path, line_count, lines_unique, compression_ratio, source_sessions, source_files, parameters 
           FROM distills 
           WHERE parameters::jsonb->>'bucket' = $1 
           ORDER BY timestamp DESC 
           LIMIT 50`, [bucketParam]);
...
export async function smartChatSearch(query, buckets = [], maxChars = 20000, tags = [], provenance = 'all', useMaxRecall = false, userContext, mode = 'standard') {
    ...
    for (const entity of splitQueries) {
        parallelResults.push(await executeSearch(entity, buckets, budgetPerQuery, provenance, tags, undefined, useMaxRecall, userContext, mode));
    }
    // 4. Merge & Deduplicate
    ...
    if (useMaxRecall && mergedResults.length > 0) {
        ...
        const inflatedResults = await ContextInflator.inflate(mergedResults, maxChars, budgetPerAtom);
