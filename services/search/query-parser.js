 ...
 */
export async function getGlobalTags(limit = 50) {
    try {
 ...
 */
export function expandCamelCase(terms) {
 ...
 */
export function extractTemporalContext(query) {
    const now = new Date();
    ...
    if (match) {
        ...
        if (unit.startsWith('year')) {
            ...
            if (pastYear < currentYear) {
                for (let y = pastYear; y < currentYear; y++)
                    tags.add(y.toString());
            }
        }
    }
 ...
 */
export function extractKeyTermsFromConversation(conversation) {
    const doc = nlp.readDoc(conversation);
    // Extract named entities (people, places, organizations)
    const entities = doc.entities().out(nlp.its.normal);
    // Extract important tokens (nouns, proper nouns, adjectives, verbs)
    const importantTokens = doc.tokens().filter((t) => {
        const tag = t.out(nlp.its.pos);
        const text = t.out().toLowerCase();
        // Include named entities
        if (entities.includes(text))
            return true;
        // Include important POS tags
        return tag === 'NOUN' || tag === 'PROPN' || tag === 'ADJ' || tag === 'VERB';
    }).out(nlp.its.normal);
    // Combine and deduplicate
    const allTerms = [...new Set([...entities, ...importantTokens])];
 ...
 */
export function splitQueryIntoMolecules(query) {
...
export function parseQuery(query) {
    const result = { phrases: [], temporal: [], buckets: [], keywords: [] };
    const phraseRegex = /"([^"]+)"/g;
    let phraseMatch;
 ...
 */
export function expandConversationalQuery(query) {
    const expansions = [];
 ...
 */
export async function getRelatedTagsForQuery(query, maxTags = 5) {
    try {
