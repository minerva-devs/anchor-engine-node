...
export async function createMirror() {
    ...
    for (const row of rows) {
        const dbPath = Array.isArray(row) ? row[0] : row.path;
        if (!dbPath)
            continue;
 ...
 */
export async function writeMirroredFile(relativePath, content, provenance = 'internal') {
 ...
 */
export function getMirrorPath(sourcePath, provenance = 'internal') {
