 ...
 */
export async function batchFetchCompounds(compoundIds) {
    ...
    const result = await db.run(`SELECT compound_id AS id, source_path AS path, provenance
     FROM molecules
     WHERE compound_id = ANY($1)`, [uniqueIds]);
