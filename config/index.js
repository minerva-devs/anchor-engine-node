...
const ServerSettingsSchema = z.object({
    ...
    api_key: z.string()
        ...
        .regex(/^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)[a-zA-Z0-9]{32,128}$|^[a-f0-9]{64,}$/i, 'API key must be 32-128 alphanumeric chars with mixed case+digit - OR 64+ char hex')
        .optional(),
});
...
const DEFAULT_CONFIG = {
    ...
    INGESTION: {
        CONCEPT_DENSITY: process.env['ANCHOR_CONCEPT_DENSITY'] || 'medium',
        TAG_THRESHOLD: parseFloat(process.env['ANCHOR_TAG_THRESHOLD'] || '0.7'),
        DEDUP_STRENGTH: process.env['ANCHOR_DEDUP_STRENGTH'] || 'medium',
        TOKEN_BUDGET_DEFAULT: parseInt(process.env['ANCHOR_TOKEN_BUDGET_DEFAULT'] || '2000', 10),
        INGESTION_PROFILE: process.env['ANCHOR_INGESTION_PROFILE'] || 'default',
    },
};
...
function loadConfig() {
    ...
    if (fs.existsSync(anchorSettingsPath) && fs.statSync(anchorSettingsPath).isFile()) {
        try {
            ...
            if (userSettings.llm) {
                // Provider settings
                if (userSettings.llm.provider)
                    loadedConfig.LLM_PROVIDER = userSettings.llm.provider;
                if (userSettings.llm.remote_url)
                    loadedConfig.REMOTE_LLM_URL = userSettings.llm.remote_url;
                if (userSettings.llm.remote_model)
                    loadedConfig.REMOTE_MODEL_NAME = userSettings.llm.remote_model;
                if (userSettings.llm.model_dir)
                    loadedConfig.LLM_MODEL_DIR = userSettings.llm.model_dir;
                // Main model
                if (userSettings.llm.chat_model)
                    loadedConfig.MODELS.MAIN.PATH = userSettings.llm.chat_model;
                if (userSettings.llm.gpu_layers !== undefined)
                    loadedConfig.MODELS.MAIN.GPU_LAYERS = userSettings.llm.gpu_layers;
                if (userSettings.llm.ctx_size !== undefined)
                    loadedConfig.MODELS.MAIN.CTX_SIZE = userSettings.llm.ctx_size;
                if (userSettings.llm.max_tokens !== undefined)
                    loadedConfig.MODELS.MAIN.MAX_TOKENS = userSettings.llm.max_tokens;
                // Orchestrator model
                if (userSettings.llm.task_model)
                    loadedConfig.MODELS.ORCHESTRATOR.PATH = userSettings.llm.task_model;
                if (userSettings.llm.orchestrator_ctx_size !== undefined)
                    loadedConfig.MODELS.ORCHESTRATOR.CTX_SIZE = userSettings.llm.orchestrator_ctx_size;
                if (userSettings.llm.orchestrator_gpu_layers !== undefined)
                    loadedConfig.MODELS.ORCHESTRATOR.GPU_LAYERS = userSettings.llm.orchestrator_gpu_layers;
                if (userSettings.llm.orchestrator_max_tokens !== undefined)
                    loadedConfig.MODELS.ORCHESTRATOR.MAX_TOKENS = userSettings.llm.orchestrator_max_tokens;
                // Vision model
                if (userSettings.llm.vision_model)
                    loadedConfig.MODELS.VISION.PATH = userSettings.llm.vision_model;
                if (userSettings.llm.vision_projector)
                    loadedConfig.MODELS.VISION.PROJECTOR = userSettings.llm.vision_projector;
                if (userSettings.llm.vision_ctx_size !== undefined)
                    loadedConfig.MODELS.VISION.CTX_SIZE = userSettings.llm.vision_ctx_size;
                if (userSettings.llm.vision_gpu_layers !== undefined)
                    loadedConfig.MODELS.VISION.GPU_LAYERS = userSettings.llm.vision_gpu_layers;
                if (userSettings.llm.vision_max_tokens !== undefined)
                    loadedConfig.MODELS.VISION.MAX_TOKENS = userSettings.llm.vision_max_tokens;
            }
            // Load Search Settings (single consolidated block)
            if (userSettings.search) {
                if (userSettings.search.strategy)
                    loadedConfig.SEARCH.strategy = userSettings.search.strategy;
                if (userSettings.search.hide_years_in_tags !== undefined)
                    loadedConfig.SEARCH.hide_years_in_tags = userSettings.search.hide_years_in_tags;
                if (userSettings.search.whitelist)
                    loadedConfig.SEARCH.whitelist = userSettings.search.whitelist;
                if (userSettings.search.max_chars_default !== undefined)
                    loadedConfig.SEARCH.max_chars_default = userSettings.search.max_chars_default;
                if (userSettings.search.max_chars_limit !== undefined)
                    loadedConfig.SEARCH.max_chars_limit = userSettings.search.max_chars_limit;
                if (userSettings.search.fts_window_size !== undefined)
                    loadedConfig.SEARCH.fts_window_size = userSettings.search.fts_window_size;
                if (userSettings.search.fts_padding !== undefined)
                    loadedConfig.SEARCH.fts_padding = userSettings.search.fts_padding;
            }
            // Load Server Settings
            if (userSettings.server) {
                if (userSettings.server.host)
                    loadedConfig.HOST = userSettings.server.host;
                if (userSettings.server.port)
                    loadedConfig.PORT = userSettings.server.port;
                if (userSettings.server.api_key !== undefined)
                    loadedConfig.API_KEY = userSettings.server.api_key;
                if (userSettings.server.version !== undefined)
                    loadedConfig.VERSION = userSettings.server.version;
                if (userSettings.github?.token !== undefined)
                    loadedConfig.GITHUB_TOKEN = userSettings.github.token;
            }
            // Load Resource Management Settings
            if (userSettings.resource_management) {
                if (userSettings.resource_management.gc_cooldown_ms !== undefined)
                    loadedConfig.GC_COOLDOWN_MS = userSettings.resource_management.gc_cooldown_ms;
                if (userSettings.resource_management.max_atoms_in_memory !== undefined)
                    loadedConfig.MAX_ATOMS_IN_MEMORY = userSettings.resource_management.max_atoms_in_memory;
                if (userSettings.resource_management.monitoring_interval_ms !== undefined)
                    loadedConfig.MONITORING_INTERVAL_MS = userSettings.resource_management.monitoring_interval_ms;
                if (userSettings.resource_management.cache_ttl_ms !== undefined)
                    loadedConfig.CACHE_TTL_MS = userSettings.resource_management.cache_ttl_ms;
                if (userSettings.resource_management.max_cache_size !== undefined)
                    loadedConfig.MAX_CACHE_SIZE = userSettings.resource_management.max_cache_size;
            }
            // Load Watcher Settings
            if (userSettings.watcher) {
                if (userSettings.watcher.debounce_ms !== undefined)
                    loadedConfig.WATCHER_DEBOUNCE_MS = userSettings.watcher.debounce_ms;
                if (userSettings.watcher.stability_threshold_ms !== undefined)
                    loadedConfig.WATCHER_STABILITY_THRESHOLD_MS = userSettings.watcher.stability_threshold_ms;
                if (userSettings.watcher.extra_paths)
                    loadedConfig.WATCHER_EXTRA_PATHS = userSettings.watcher.extra_paths;
                if (userSettings.watcher.auto_start !== undefined)
                    loadedConfig.WATCHER_AUTO_START = userSettings.watcher.auto_start;
                if (userSettings.watcher.wipe_mirrored_brain_on_shutdown !== undefined)
                    loadedConfig.WATCHER_WIPE_MIRRORED_BRAIN_ON_SHUTDOWN = userSettings.watcher.wipe_mirrored_brain_on_shutdown;
            }
            // Load Context Relevance Settings
            if (userSettings.context) {
                if (userSettings.context.relevance_weight !== undefined)
                    loadedConfig.CONTEXT_RELEVANCE_WEIGHT = userSettings.context.relevance_weight;
                if (userSettings.context.recency_weight !== undefined)
                    loadedConfig.CONTEXT_RECENCY_WEIGHT = userSettings.context.recency_weight;
            }
            // Load Service Settings
            if (userSettings.services) {
                if (userSettings.services.vision_server_port !== undefined)
                    loadedConfig.SERVICES.VISION_SERVER_PORT = userSettings.services.vision_server_port;
                if (userSettings.services.chat_server_port !== undefined)
                    loadedConfig.SERVICES.CHAT_SERVER_PORT = userSettings.services.chat_server_port;
                if (userSettings.services.tag_infector_unload_timeout !== undefined)
                    loadedConfig.SERVICES.TAG_INFECTOR_UNLOAD_TIMEOUT = userSettings.services.tag_infector_unload_timeout;
                if (userSettings.services.tag_gliner_check_interval !== undefined)
                    loadedConfig.SERVICES.TAG_GLINER_CHECK_INTERVAL = userSettings.services.tag_gliner_check_interval;
            }
            // Load Database Settings
            if (userSettings.database) {
                if (userSettings.database.wipe_on_startup !== undefined)
                    loadedConfig.DATABASE.WIPE_ON_STARTUP = userSettings.database.wipe_on_startup;
                if (userSettings.database.wipe_on_shutdown !== undefined)
                    loadedConfig.DATABASE.WIPE_ON_SHUTDOWN = userSettings.database.wipe_on_shutdown;
                if (userSettings.database.shared_buffers_mb !== undefined)
                    loadedConfig.DATABASE.SHARED_BUFFERS_MB = userSettings.database.shared_buffers_mb;
                if (userSettings.database.effective_cache_size_mb !== undefined)
                    loadedConfig.DATABASE.EFFECTIVE_CACHE_SIZE_MB = userSettings.database.effective_cache_size_mb;
                if (userSettings.database.work_mem_mb !== undefined)
                    loadedConfig.DATABASE.WORK_MEM_MB = userSettings.database.work_mem_mb;
                if (userSettings.database.maintenance_work_mem_mb !== undefined)
                    loadedConfig.DATABASE.MAINTENANCE_WORK_MEM_MB = userSettings.database.maintenance_work_mem_mb;
            }
            // Load Limits and Thresholds
            if (userSettings.limits) {
                if (userSettings.limits.max_file_size_bytes !== undefined)
                    loadedConfig.LIMITS.MAX_FILE_SIZE_BYTES = userSettings.limits.max_file_size_bytes;
                if (userSettings.limits.max_content_length_chars !== undefined)
                    loadedConfig.LIMITS.MAX_CONTENT_LENGTH_CHARS = userSettings.limits.max_content_length_chars;
                if (userSettings.limits.max_chunk_size_chars !== undefined)
                    loadedConfig.LIMITS.MAX_CHUNK_SIZE_CHARS = userSettings.limits.max_chunk_size_chars;
                if (userSettings.limits.max_summary_length_chars !== undefined)
                    loadedConfig.LIMITS.MAX_SUMMARY_LENGTH_CHARS = userSettings.limits.max_summary_length_chars;
                if (userSettings.limits.date_extractor_scan_limit !== undefined)
                    loadedConfig.LIMITS.DATE_EXTRACTOR_SCAN_LIMIT = userSettings.limits.date_extractor_scan_limit;
            }
            // Load Memory Management Settings (Standard 127/134/135)
            if (userSettings.memory) {
                if (userSettings.memory.heap_pressure_mb !== undefined)
                    loadedConfig.MEMORY.HEAP_PRESSURE_MB = userSettings.memory.heap_pressure_mb;
                if (userSettings.memory.throttle_start_mb !== undefined)
                    loadedConfig.MEMORY.THROTTLE_START_MB = userSettings.memory.throttle_start_mb;
                if (userSettings.memory.throttle_max_mb !== undefined)
                    loadedConfig.MEMORY.THROTTLE_MAX_MB = userSettings.memory.throttle_max_mb;
                if (userSettings.memory.emergency_stop_mb !== undefined)
                    loadedConfig.MEMORY.EMERGENCY_STOP_MB = userSettings.memory.emergency_stop_mb;
                if (userSettings.memory.search_results_batch_size !== undefined)
                    loadedConfig.MEMORY.SEARCH_RESULTS_BATCH_SIZE = userSettings.memory.search_results_batch_size;
                if (userSettings.memory.enable_streaming_results !== undefined)
                    loadedConfig.MEMORY.ENABLE_STREAMING_RESULTS = userSettings.memory.enable_streaming_results;
            }
            // Load Low-Resource Mode Settings (Standard 136)
            if (userSettings.low_resource) {
                if (userSettings.low_resource.enabled !== undefined)
                    loadedConfig.LOW_RESOURCE.ENABLED = userSettings.low_resource.enabled;
                if (userSettings.low_resource.max_recall_low !== undefined)
                    loadedConfig.LOW_RESOURCE.MAX_RECALL_LOW = userSettings.low_resource.max_recall_low;
                if (userSettings.low_resource.max_recall_high !== undefined)
                    loadedConfig.LOW_RESOURCE.MAX_RECALL_HIGH = userSettings.low_resource.max_recall_high;
                if (userSettings.low_resource.max_concurrency !== undefined)
                    loadedConfig.LOW_RESOURCE.MAX_CONCURRENCY = userSettings.low_resource.max_concurrency;
                if (userSettings.low_resource.search_batch_size !== undefined)
                    loadedConfig.LOW_RESOURCE.SEARCH_BATCH_SIZE = userSettings.low_resource.search_batch_size;
                if (userSettings.low_resource.ingest_batch_size !== undefined)
                    loadedConfig.LOW_RESOURCE.INGEST_BATCH_SIZE = userSettings.low_resource.ingest_batch_size;
                if (userSettings.low_resource.vector_ingest_batch !== undefined)
                    loadedConfig.LOW_RESOURCE.VECTOR_INGEST_BATCH = userSettings.low_resource.vector_ingest_batch;
            }
            // Load Ingestion Configuration (Agent-Controlled)
            if (userSettings.ingestion) {
                if (userSettings.ingestion.concept_density)
                    loadedConfig.INGESTION.CONCEPT_DENSITY = userSettings.ingestion.concept_density;
                if (userSettings.ingestion.tag_threshold !== undefined)
                    loadedConfig.INGESTION.TAG_THRESHOLD = userSettings.ingestion.tag_threshold;
                if (userSettings.ingestion.dedup_strength)
                    loadedConfig.INGESTION.DEDUP_STRENGTH = userSettings.ingestion.dedup_strength;
                if (userSettings.ingestion.token_budget_default !== undefined)
                    loadedConfig.INGESTION.TOKEN_BUDGET_DEFAULT = userSettings.ingestion.token_budget_default;
                if (userSettings.ingestion.ingestion_profile)
                    loadedConfig.INGESTION.INGESTION_PROFILE = userSettings.ingestion.ingestion_profile;
            }
            // Load Distiller Configuration (v2.1)
            if (userSettings.distiller) {
                if (userSettings.distiller.similarity_threshold !== undefined)
                    loadedConfig.DISTILLER.SIMILARITY_THRESHOLD = userSettings.distiller.similarity_threshold;
                if (userSettings.distiller.output_format)
                    loadedConfig.DISTILLER.OUTPUT_FORMAT = userSettings.distiller.output_format;
            }
        }
        catch (e) {
            ...
            if (e instanceof z.ZodError) {
                ...
                const issues = e.issues || e.flatten?.().errors || [];
                if (issues.length > 0) {
                    issues.forEach((err) => {
