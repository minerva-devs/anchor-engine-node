...
const ANCHOR_ROOT = path.resolve(process.env.ANCHOR_ROOT ||
...
// Priority: Environment variables > user_settings.json > defaults
export const NOTEBOOK_DIR = path.resolve(process.env.NOTEBOOK_DIR ||
    userSettings.paths?.notebook ||
    path.join(LOCAL_DATA_DIR, 'notebook'));
export const CONTEXT_DIR = path.resolve(process.env.CONTEXT_DIR ||
...
export const { BACKUPS_DIR, EXTERNAL_INBOX_DIR, DISTILLS_DIR, MIRRORED_BRAIN_DIR, ENGINE_CONTEXT, ENGINE_PLUGINS, DESKTOP_OVERLAY_SRC, DESKTOP_OVERLAY_DIST, } = PATHS;
// Export provenance configuration
export const DEFAULT_PROVENANCE = path.resolve(process.env.DEFAULT_PROVENANCE ||
