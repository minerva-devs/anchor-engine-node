...
def check_url(url, service_name, timeout=10):
    ...
    try:
        ...
        if response.status_code < 500:  # Consider it available if not a server error
            ...
            print(f"✗ {service_name} returned status code {response.status_code} at {url}")
            return False
...
def check_required_services():
    ...
    # Check Neo4j
    neo4j_uri = os.getenv('NEO4J_URI', config.get('neo4j', {}).get('uri', 'neo4j://localhost:7687'))
    neo4j_user = os.getenv('NEO4J_USER', config.get('neo4j', {}).get('user', 'neo4j'))
    neo4j_password = os.getenv('NEO4J_PASSWORD', config.get('neo4j', {}).get('password', 'password'))
    ...
    if all_ok:
        ...
        if not llm_ok:
            print(f"  - LLM Service ({active_provider}): Ensure the {active_provider} server is running")
        if not utcp_ok:
