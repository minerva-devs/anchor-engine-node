        ...
        <div className={`p-4 bg-slate-900 border-l-4 rounded-md mb-4 shadow-sm ${getCategoryColor(rule.category)}`}>
            ...
            <div className="mb-4">
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                    {rule.triggers.map((t: string) => (
                        <span key={t} className="px-2 py-1 bg-slate-800 rounded text-sm text-slate-200 flex items-center gap-1 border border-slate-700">
                            {t}
                            <Button variant="icon" aria-label="Remove trigger" onClick={() => removeTrigger(t)} style={{ color: '#f87171' }}><X size={12} /></Button>
                        </span>
                    ))}
                </div>
                <div className="flex gap-2">
                    ...
                    />
                    <Button variant="icon" aria-label="Add trigger" onClick={addTrigger} className="bg-slate-800 rounded text-slate-400" style={{ padding: '4px' }}><Plus size={16} /></Button>
                </div>
            </div>
                ...
                <div className="flex flex-wrap gap-2 mb-2">
                    {rule.exclusions.map((e: string) => (
                        <span key={e} className="px-2 py-1 bg-red-900/20 rounded text-sm text-red-300 flex items-center gap-1 border border-red-900/50">
                            {e}
                            <Button variant="icon" aria-label="Remove exclusion" onClick={() => removeExclusion(e)} style={{ color: '#fca5a5' }}><X size={12} /></Button>
                        </span>
                    ))}
                </div>
                <div className="flex gap-2">
                    ...
                    />
                    <Button variant="icon" aria-label="Add exclusion" onClick={addExclusion} className="bg-slate-800 rounded text-slate-400" style={{ padding: '4px' }}><Plus size={16} /></Button>
                </div>
            </div>
        </div>
    );
