...
async function callAnchorAPI(endpoint, method = 'GET', body) {
    ...
    if (ANCHOR_API_KEY) {
        options.headers.Authorization = `Bearer ${ANCHOR_API_KEY}`;
    }
    if (body && method !== 'GET') {
...
async function handleQuery(args) {
    ...
    if (Array.isArray(results)) {
        ...
        return allResults
            .map((r, i) => `[${i + 1}] Score: ${(r.score || 0).toFixed(3)} | Source: ${r.source || 'unknown'}\n${r.content?.substring(0, 500) || '[no content]'}${r.content?.length > 500 ? '...' : ''}`)
            .join('\n\n---\n\n');
    }
...
async function handleIlluminate(args) {
...
async function handleListCompounds(args) {
    const { filter = '', limit = 50 } = args;
    const result = await callAnchorAPI('/v1/compounds/list', 'GET');
    let compounds = result.compounds || [];
    if (filter) {
        compounds = compounds.filter((c) => c.name?.toLowerCase().includes(filter.toLowerCase()));
    }
    ...
    return compounds
        .map((c) => `- ${c.name} (${c.molecule_count || 0} molecules, ${c.atom_count || 0} atoms)`)
        .join('\n');
}
...
async function handleIngestFile(args) {
    // Security check: Write operations must be enabled
    if (!securitySettings.allow_write_operations) {
        return `❌ Write operations are disabled.
...
async function handleGithubIngest(args) {
    const { url, branch = 'main', bucket = 'code', run_analysis = false, include_history = true, } = args;
    if (!url) {
    ...
    try {
        ...
        const features = [];
        if (include_history)
            features.push('📝 commit history');
        if (run_analysis)
            features.push('🔍 code analysis');
...
async function handleWatchdogStart(args) {
    try {
...
async function handleWatchdogStop(args) {
    try {
...
async function handleWatchdogStatus(args) {
    try {
...
async function handleWatchdogIngest(args) {
    try {
...
// Resource handlers
async function handleResourceStats() {
    const result = await callAnchorAPI('/v1/stats', 'GET');
    return JSON.stringify(result, null, 2);
}
async function handleResourceCompounds() {
    const result = await callAnchorAPI('/v1/compounds/list', 'GET');
