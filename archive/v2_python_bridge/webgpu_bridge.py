...
async def lifespan(app: FastAPI):
    ...
    # Check if resurrection mode is disabled via config or environment
    resurrection_enabled = config.get("ghost_engine.auto_resurrection_enabled", True)
    ...
    if resurrection_enabled:
        ...
        await add_log_entry("Resurrection", "info", "No Resurrection Mode Enabled - Ghost Engine must be started manually")
        await add_log_entry("System", "info", "💡 Open ghost.html in your browser to connect to the Bridge")

    yield

    # Shutdown
    await add_log_entry("System", "info", "Shutting down Anchor Core...")
    # Only terminate browser if resurrection mode is enabled
    if resurrection_enabled:
...
class ResurrectionManager:
    ...
    def __init__(self):
        ...
        self.max_retries = config.get("ghost_engine.max_retries", 3)
        self.retry_delay = config.get("ghost_engine.retry_delay", 2)  # seconds
    ...
    def _get_launch_command(self) -> list:
        ...
        if sys.platform == "win32":
            # Detect Edge Path from configuration
            edge_path = "msedge" # Fallback
            possible_paths = config.get("ghost_engine.browser_executables", [
            ...
            for path in possible_paths:
                # Check if it's a full path or just an executable name
                if path.startswith("C:"):
                    if os.path.exists(path):
                        edge_path = path
                        break
                else:
                    # For executable names, try to find in PATH
                    import shutil
                    if shutil.which(path):
                        edge_path = path
                        break

            # Base Windows Command from configuration
            base_cmd = [edge_path]
            base_cmd.extend(config.get("ghost_engine.browser_flags", [
                "--headless=new",
                "--no-first-run",
            ...
            # Update the user data dir with current timestamp
            base_cmd = [arg if not arg.startswith("--user-data-dir=") else f"--user-data-dir={temp_dir}/anchor_ghost_{int(time.time())}" for arg in base_cmd]

            # Append mode-specific flags from configuration
            if os.environ.get("LOW_RESOURCE_MODE") == "true":
                base_cmd.extend(config.get("ghost_engine.low_resource_flags", [
                ...
                base_cmd.extend(config.get("ghost_engine.cpu_only_flags", [
                    "--disable-gpu",
            ...
            # Linux/Mac: Use Chrome with configuration
            chrome_cmd = ["google-chrome"]
            chrome_cmd.extend(config.get("ghost_engine.browser_flags", [
                "--headless",
                "--no-first-run",
                "--disable-background-timer-throttling",
                "--disable-backgrounding-occluded-windows",
            ...
            # Update the user data dir with current timestamp
            chrome_cmd = [arg if not arg.startswith("--user-data-dir=") else f"--user-data-dir={temp_dir}/anchor_ghost_{int(time.time())}" for arg in chrome_cmd]

            chrome_cmd.append(f"http://localhost:{PORT}/ghost.html?headless=true")
            return chrome_cmd
    ...
    async def launch_browser(self):
        ...
        # Get fresh launch command each time to ensure correct path detection
        ...
        try:
            ...
            # Update the stored command for potential later use
...
async def memory_ingest(request: Request):
    ...
    try:
        ...
        # Wait for acknowledgment (with timeout from configuration)
        timeout = config.get("memory.ingest_timeout", 10.0)
        result = await asyncio.wait_for(active_requests[req_id].get(), timeout=timeout)
        del active_requests[req_id]
