#!/usr/bin/env python3
"""
Gemma-3-4b-it-MAX-HORROR Server Launcher with Auto-Download

Downloads the Gemma-3-4b-it-MAX-HORROR model and starts the llama-server
with optimized settings for creative writing and maximum context window.
"""
import os
import sys
import subprocess
...
def find_gemma3_model():
    """Find the Gemma-3-4b-it-MAX-HORROR model, downloading if necessary."""
    ...
    for models_dir in possible_dirs:
        if models_dir.exists():
            ...
            # Look for Gemma-3-4b-it-MAX-HORROR model with flexible matching
            # Using more flexible search to match your actual model names like "Gemma3-4B-Arceus-Servant.i1-Q6_K.gguf"
            for pattern in ["*Gemma*3*4b*MAX*HORROR*.gguf", "*gemma*3*4b*max*horror*.gguf", "*Gemma*3*4B*MAX*HORROR*.gguf", "*Maximizing*Model*Performance*.gguf", "*Gemma*3*4b*MAX*HORROR*DAU*.gguf", "*Gemma*3*4b*MAX*HORROR*D_AU*.gguf", "*Gemma*MAX*HORROR*DAU*.gguf"]:
    ...
    if model_files:
        ...
        print(f"[FOUND] Gemma-3-4b-it-MAX-HORROR model found: {matched_model}")
    ...
    print("[INFO] Gemma-3-4b-it-MAX-HORROR model not found, starting download...")

    # Try multiple URLs - in case the original repo is not accessible
    possible_downloads = [
        # Original repo with various quantization options - based on actual model naming
        ("https://huggingface.co/DavidAU/Gemma-3-4b-it-MAX-HORROR-Uncensored-DBL-X-Imatrix-GGUF/resolve/main/", [
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-DBL-X-Imatrix-Q8_0.gguf",
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-DBL-X-Imatrix-Q5_K_M.gguf",
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-DBL-X-Imatrix-Q4_K_M.gguf",
            "gemma-3-4b-it-MAX-HORROR-Q8_0.gguf",
            "gemma-3-4b-it-MAX-HORROR-Q5_K_M.gguf",
            "gemma-3-4b-it-MAX-HORROR-Q4_K_M.gguf",
            "gemma-3-4b-it-MAX-HORROR.gguf"
        ]),
        # DavidAU's alternative model repository with different naming pattern (matches your existing model)
        ("https://huggingface.co/DavidAU/Gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-GGUF/resolve/main/", [
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-Q8_0-imat.gguf",
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-Q5_K_M-imat.gguf",
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-Q4_K_M-imat.gguf",
            "Gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-F16-imat.gguf",
            "gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-Q8_0-imat.gguf",
            "gemma-3-4b-it-MAX-HORROR-Uncensored-D_AU-Q5_K_M-imat.gguf"
        ]),
        # Alternative gemma-3 repositories if the original doesn't work
        ("https://huggingface.co/bartowski/Gemma-3-4B-It-MAX-HORROR-Uncensored-DBL-X-Imatrix-GGUF/resolve/main/", [
            "Gemma-3-4B-It-MAX-HORROR-Uncensored-DBL-X-Imatrix-GGUF-Q8_0.gguf",
            "Gemma-3-4B-It-MAX-HORROR-Uncensored-DBL-X-Imatrix-GGUF-Q5_K_M.gguf",
            "Gemma-3-4B-It-MAX-HORROR-Uncensored-DBL-X-Imatrix-GGUF-Q4_K_M.gguf",
        ]),
        # Additional model from DavidAU with various quantization types
        ("https://huggingface.co/DavidAU/Maximizing-Model-Performance-All-Quants-Types-And-Full-Precision-by-Samplers_Parameters/resolve/main/", [
            "model-Q8_0.gguf",
            "model-Q6_K.gguf",
            "model-Q5_K_M.gguf",
            "model-Q4_K_M.gguf",
...
def get_optimal_settings(model_char, system_specs):
    ...
    if model_char['size_gb'] <= 2.0:
        ...
        settings['ctx_size'] = 65536   # 64k for 4B models like Gemma-3
...
def check_flash_attention_support(llama_server_path):
    ...
    # Test the command-line flag directly by checking for the specific error message
    try:
        # Test with a minimal call that should fail due to missing required args,
        # but will reveal if --fa is recognized or not
        result = subprocess.run([str(llama_server_path), "--fa", "--version"],
                              capture_output=True, text=True, timeout=5)
        error_output = result.stderr.lower() if result.stderr else ""

        # If the error contains "invalid argument: --fa" or similar, then flag is NOT supported
        if "invalid argument: --fa" in error_output or "unknown option: --fa" in error_output:
            return False
        elif "unrecognized" in error_output and "--fa" in error_output:
            return False
        else:
            # If no error about --fa being invalid, assume it's supported
            # (though it may fail for other reasons like missing model)
            return True
...
def start_llm_server(model_path, port=8080):
    """Start llama-server with Gemma-3-4b-it-MAX-HORROR optimized settings."""
    ...
    # Gemma-3-4b-it-MAX-HORROR optimized settings for creative writing and long contexts (64k)
    cmd = [
        str(llama_server),
        "-m", str(model_path),
        "--port", str(port),
        ...
        "--mirostat", "2",          # Advanced sampling for creative models
        "--temp", "1.2",            # Higher temperature for creative outputs
        "--top-p", "0.95",          # Nucleus sampling threshold
        "--cache-type-k", "q8_0",   # Quantized KV cache to save RAM
        "--cache-type-v", "q8_0",   # Quantized KV cache to save RAM
        ...
        "--pooling", "cls",         # Specific pooling for Gemma models
        "--rope-freq-base", str(optimal_settings['rope_freq_base']), # Gemma-specific frequency base
        # "--chat-template", "gemma", # REMOVED: Explicitly set Gemma chat template to prevent role alternation errors
    ]
    ...
    try:
        ...
        print(f"   - Role: Creative Engine (Dark Artist Mode)")
...
def main():
    parser = argparse.ArgumentParser(description="Gemma-3-4b-it-MAX-HORROR Server Launcher (Auto-Download & Creative Optimization)")
    parser.add_argument("--port", type=int, default=8080, help="Port for llama-server (default: 8080)")
    parser.add_argument("--model", type=str, help="Path to model file (defaults to Gemma-3-4b-it-MAX-HORROR auto-download)")
    ...
    if args.model:
        ...
        if not model_path:
            print("[ERROR] Could not find or download Gemma-3-4b-it-MAX-HORROR model, exiting")
            return
