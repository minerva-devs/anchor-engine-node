...
class FakeLLM:
    async def generate(self, prompt, system_prompt=None, **kwargs):
        return "[FAKE-LLM-FINAL] Response after tool returned nothing"
...
class FakePluginManagerEmptyResult:
    def __init__(self):
        self.enabled = True
    ...
    async def execute_tool(self, name, **kwargs):
        # Simulate plugin that returns no content (None)
        return None
...
def test_chat_stream_handles_empty_tool_result(monkeypatch):
    app = create_app_with_routers()

    with TestClient(app) as client:
        client.app.state.llm = FakeLLM()
        client.app.state.plugin_manager = FakePluginManagerEmptyResult()
    ...
    # Our fallback behavior should still include the standardized marker
