...
from src.content_utils import clean_content, is_json_like, is_html_like, is_token_soup, sanitize_token_soup
...
# default EMB_MAX_CHARS is driven by global_settings.llm_embeddings_chunk_size_default (settable via .env)


async def embed_texts(client: LLMClient, texts: list[str]):
    # returns list of embeddings
    # Use chunk-aware per-document embedding helper, to avoid server 500s for long texts
...
async def run_repair(window_async=False, threshold=0.75, limit=100, candidate_limit: int | None = None, dry_run=True, csv_out=None, batch_size: int | None = None, min_batch=1, embed_delay=0.15, embed_retries=3, emb_max_chars: int | None = None, top_n=1, skip_json=True, skip_html=True, min_clean_length=30, min_origin_length=100, time_window_hours=None, prefer_same_app=False, require_same_app=False, delta=None, max_commit=None, commit=False, exclude_phrases=None, skip=0, run_id: str = None, disable_chunking: bool = False, export_top: int | None = None, exclude_tag: str | None = None):
    ...
    with driver.session() as session:
        # Optionally exclude nodes with a specific tag (e.g., '#corrupted') to avoid token-soup content
        exclude_tag_clause = ''
        params_tags = {'limit': limit, 'skip': skip}
        if exclude_tag:
            # We'll detect both plain and JSON-string tags; matching via toLower
            exclude_tag_clause = " AND NOT (s.tags IS NOT NULL AND ANY(t IN s.tags WHERE toLower(t) = $exclude_tag))"
            params_tags['exclude_tag'] = exclude_tag.lower()
        results = session.run("""
            MATCH (s:Memory)
            WHERE s.category='summary' AND NOT (s)-[:DISTILLED_FROM]->()""" + exclude_tag_clause + """
        ...
        for sr in summaries:
            ...
            # if exclude_tag set, add clause to candidate query to skip tagged nodes
            if exclude_tag:
                # Insert the exclude expression just before the RETURN keyword, preserving the original query
                exclude_expr = " AND NOT (orig.tags IS NOT NULL AND ANY(t IN orig.tags WHERE toLower(t) = $exclude_tag)) "
                q_upper = candidates_query.upper()
                if 'WHERE' in q_upper and 'RETURN' in q_upper:
                    idx_where = q_upper.find('WHERE')
                    idx_return = q_upper.find('RETURN', idx_where)
                    if idx_return > idx_where:
                        candidates_query = candidates_query[:idx_return] + exclude_expr + candidates_query[idx_return:]
                else:
                    # fallback to simple append if structure is unexpected
                    candidates_query = candidates_query.replace('RETURN', exclude_expr + ' RETURN')
                params['exclude_tag'] = exclude_tag.lower()
            ...
            if prefer_same_app and time_window_hours and sr.get('s_app_id') and sr.get('s_created_at'):
                ...
                if exclude_tag:
                    exclude_expr = " AND NOT (orig.tags IS NOT NULL AND ANY(t IN orig.tags WHERE toLower(t) = $exclude_tag)) "
                    idx_return = q_same.upper().find('RETURN')
                    if idx_return > -1:
                        q_same = q_same[:idx_return] + exclude_expr + q_same[idx_return:]
                    params_same['exclude_tag'] = exclude_tag.lower()
                cres_same = session.run(q_same, params_same)
                ...
                if len(candidates) < candidate_limit:
                    ...
                    params_other = {'min_dt': min_dt, 'max_dt': max_dt, 'remaining': remaining, 's_app_id': sr.get('s_app_id')}
                    if exclude_tag:
                        exclude_expr = " AND NOT (orig.tags IS NOT NULL AND ANY(t IN orig.tags WHERE toLower(t) = $exclude_tag)) "
                        idx_return = q_other.upper().find('RETURN')
                        if idx_return > -1:
                            q_other = q_other[:idx_return] + exclude_expr + q_other[idx_return:]
                        params_other['exclude_tag'] = exclude_tag.lower()
                    cres_other = session.run(q_other, params_other)
            ...
            try:
                ...
                # Token-soup detection & sanitization: try to sanitize and, if unsuccessful skip
                try:
                    if is_token_soup(s_content or ''):
                        print(f"⚠️  Detected token-soup for summary {s_eid}, attempting to sanitize")
                        s_sanitized = sanitize_token_soup(s_content or '') or ''
                        if s_sanitized and len(s_sanitized) >= (min_clean_length or 30):
                            s_content = s_sanitized
                            print(f"    ✅ Sanitized token-soup for summary {s_eid}")
                        else:
                            print(f"    ⚠️ Sanitize failed or insufficient length for summary {s_eid}; skipping summary")
                            if dry_run and csv_out:
                                append_csv(csv_out, header, [[str(run_id), str(s_eid), str(s_app_id) if s_app_id else '', str(sr.get('s_created_at') if sr.get('s_created_at') else ''), '', '', '', '', '', '', '', 'similarity_emb', 'skipped_token_soup_summary', '', (s_content or '')[:200], '', '']])
                            continue
                except Exception as e:
                    print(f"    ⚠️ Token-soup detection error for summary {s_eid}: {e}")
                s_text_to_embed = (s_content or '')
                # Use chunked embedding by default, otherwise truncation-only baseline behavior
                if disable_chunking:
                    s_text_to_embed_trunc = s_text_to_embed[:(emb_max_chars or 4096)]
                    raw_emb = await llm.get_embeddings([s_text_to_embed_trunc])
                    s_emb = raw_emb[0] if raw_emb and len(raw_emb) > 0 else None
                else:
            ...
            for r in candidates:
                ...
                # Token-soup detection for candidates: attempt sanitize; skip if not fixable
                try:
                    if is_token_soup(c_text or ''):
                        c_sanitized = sanitize_token_soup(c_text or '') or ''
                        if c_sanitized and len(c_sanitized) >= (min_origin_length or 100):
                            c_text = c_sanitized
                        else:
                            # skip candidate entirely; it's not salvageable for embedding
                            continue
                except Exception:
                    # ignore detection failures; continue with the cleaned text
                    pass
    ...
    driver.close()
    # Optional Top-N export: produce a human-readable Top-N file based on CSV if requested
    if export_top and csv_out and Path(csv_out).exists():
        try:
            # Read CSV, parse score (score at index 7 in header), sort, and write top N
            rows = []
            with open(csv_out, 'r', encoding='utf-8') as fh:
                ...
                for rr in r:
                    try:
                        score = float(rr[7]) if rr and len(rr) > 7 and rr[7] else 0.0
                    except Exception:
                        score = 0.0
                    rows.append((score, rr))
            rows = sorted(rows, key=lambda x: x[0], reverse=True)
            topn = min(int(export_top), len(rows))
            top_path = Path(csv_out).with_suffix(f".top{topn}.txt")
            with open(top_path, 'w', encoding='utf-8') as out:
                out.write(f"Top {topn} matches from {csv_out}\n")
                out.write("# score | s_eid | s_app_id | s_created_at | orig_eid | orig_app_id | orig_created_at | s_excerpt | orig_excerpt\n")
                for i in range(topn):
                    score, rr = rows[i]
                    s_eid = rr[1] if len(rr) > 1 else ''
                    s_app = rr[2] if len(rr) > 2 else ''
                    s_created = rr[3] if len(rr) > 3 else ''
                    orig_eid = rr[4] if len(rr) > 4 else ''
                    orig_app = rr[5] if len(rr) > 5 else ''
                    orig_created = rr[6] if len(rr) > 6 else ''
                    s_excerpt = rr[14] if len(rr) > 14 else ''
                    orig_excerpt = rr[15] if len(rr) > 15 else ''
                    out.write(f"{score:.4f} | {s_eid} | {s_app} | {s_created} | {orig_eid} | {orig_app} | {orig_created} | {s_excerpt} | {orig_excerpt}\n")
            print(f"Exported top {topn} file: {top_path}")
        except Exception as e:
            print(f"Failed to export top {export_top} from {csv_out}: {e}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--candidate-limit', '-c', default=None, type=int, help='Limit number of origin candidates per summary (use .env to set default)')
    ...
    parser.add_argument('--batch-size', default=None, type=int, help='Embedding batch size (if not provided, read from .env llm_embeddings_default_batch_size)')
    ...
    parser.add_argument('--disable-chunking', action='store_true', help='If set, calls to the embedding API will be truncation-only and not chunked. Useful to generate baseline/truncation CSVs for comparison (dry-run only).')
    parser.add_argument('--export-top', default=None, type=int, help='If set, export the top N rows into a human-readable file; provide N (e.g., 25)')
