...
from ece.common.token_utils import count_tokens, truncate_text_to_tokens, split_text_by_tokens
...
class TestPromptManager(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.config = PromptConfig(max_tokens=2000, reserved_tokens=100, strategy="intelligent")
...
class TestEnhancedOrchestratorAgent(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self):
        ...
        with patch('builtins.open', unittest.mock.mock_open(read_data=str(self.mock_config))):
            with patch('yaml.safe_load', return_value=self.mock_config):
                with patch('ece.agents.tier1.orchestrator.archivist_client.ArchivistClient.__init__', return_value=None):
                    with patch('ece.components.context_cache.cache_manager.CacheManager.__init__', return_value=None):
                        with patch('utcp_client.client.UTCPClient.__init__', return_value=None):
