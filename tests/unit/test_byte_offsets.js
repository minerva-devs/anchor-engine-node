...
async function testByteOffsets() {
    ...
    for (const mol of res1.molecules) {
        ...
        assert.strictEqual(slicedContent, mol.content, `Buffer slice mismatch! Expected "${mol.content}" but got "${slicedContent}"`);
    ...
    for (const mol of res2.molecules) {
        ...
        const slicedBuffer = fullBodyBuffer.subarray(mol.start_byte, mol.end_byte);
        const slicedContent = slicedBuffer.toString('utf8');
        assert.strictEqual(slicedContent, mol.content, `Buffer slice mismatch for Chinese! Expected "${mol.content}" but got "${slicedContent}"`);
        console.log('    ✅ Byte offset extraction matches!');
    }
    ...
    for (const mol of res3.molecules) {
        ...
        const slicedBuffer = fullBodyBuffer.subarray(mol.start_byte, mol.end_byte);
        const slicedContent = slicedBuffer.toString('utf8');
        assert.strictEqual(slicedContent, mol.content, `Buffer slice mismatch for mixed content! Expected "${mol.content}" but got "${slicedContent}"`);
    ...
    for (const mol of res4.molecules) {
        ...
        assert.strictEqual(normalizedSliced, normalizedMol, `Buffer slice mismatch for code! Expected "${mol.content}" but got "${slicedContent}"`);
...
//# sourceMappingURL=test_byte_offsets.js.map