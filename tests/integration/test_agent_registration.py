...
async def test_orchestrator_tool_registration():
    ...
    with patch(
        "ece.agents.tier1.orchestrator.orchestrator_agent.UTCPClient"
    ) as mock_utcp_client_class:
...
async def test_archivist_tool_registration():
    ...
    # Mock the UTCP client to capture tool registrations
    with patch(
        "ece.agents.tier3.archivist.archivist_agent.UTCPClient"
    ) as mock_utcp_client_class:
        mock_utcp_client = AsyncMock()
        mock_utcp_client_class.return_value = mock_utcp_client
        ...
        for expected_id in expected_tool_ids:
            assert expected_id in tool_ids, (
                f"Expected tool {expected_id} was not registered"
            )
...
async def test_distiller_tool_registration():
    ...
    sys.path.insert(
        0,
        os.path.join(
            os.path.dirname(__file__), "..", "..", "ece", "agents", "tier3", "distiller"
        ),
    )

    # Mock the UTCP client to capture tool registrations
    with patch(
        "ece.agents.tier3.distiller.distiller_agent.UTCPClient"
    ) as mock_utcp_client_class:
        mock_utcp_client = AsyncMock()
        mock_utcp_client_class.return_value = mock_utcp_client
...
async def test_qlearning_tool_registration():
    ...
    sys.path.insert(
        0,
        os.path.join(
            os.path.dirname(__file__), "..", "..", "ece", "agents", "tier3", "qlearning"
        ),
    )

    # Mock the UTCP client to capture tool registrations
    with patch(
        "ece.agents.tier3.qlearning.qlearning_app.UTCPClient"
    ) as mock_utcp_client_class:
        mock_utcp_client = AsyncMock()
        mock_utcp_client_class.return_value = mock_utcp_client
...
async def test_injector_tool_registration():
    ...
    sys.path.insert(
        0,
        os.path.join(
            os.path.dirname(__file__), "..", "..", "ece", "agents", "tier3", "injector"
        ),
    )

    # Mock the UTCP client to capture tool registrations
    with patch(
        "ece.agents.tier3.injector.injector_app.UTCPClient"
    ) as mock_utcp_client_class:
        mock_utcp_client = AsyncMock()
        mock_utcp_client_class.return_value = mock_utcp_client
