...
class TestPromptManagementIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_full_prompt_processing_pipeline(self, mock_conversation_respond, mock_get_context, 
                                                   mock_utcp_client, mock_cache_manager, mock_archivist_client,
                                                   mock_yaml_load, mock_open):
        ...
        mock_get_context.return_value = [{'context': 'This is a test context for the integration'}]
        mock_conversation_respond.return_value = 'This is a test response from the conversational agent.'
    ...
    @patch('ece.agents.tier1.orchestrator.archivist_client.ArchivistClient.get_context')
    @patch('ece.agents.tier2.conversational_agent.ConversationalAgent.respond')
    async def test_prompt_truncation_integration(self, mock_conversation_respond, mock_get_context,
                                                 mock_utcp_client, mock_cache_manager, mock_archivist_client,
                                                 mock_yaml_load, mock_open):
    ...
    @patch('utcp_client.client.UTCPClient.__init__')
    @patch('ece.agents.tier1.orchestrator.archivist_client.ArchivistClient.get_context')
    async def test_error_handling_in_prompt_processing(self, mock_get_context, 
                                                       mock_utcp_client, mock_cache_manager, mock_archivist_client,
                                                       mock_yaml_load, mock_open):
        ...
        self.assertIsInstance(result, str)
        self.assertIn("error", result.lower())  # Should contain some reference to an error
...
class TestPromptManagerIntegration(unittest.TestCase):
    ...
    def test_end_to_end_prompt_management(self):
        ...
        config = PromptConfig(max_tokens=4096, reserved_tokens=500, strategy="intelligent")
    ...
    def test_different_strategies(self):
        ...
        for strategy in strategies:
            config = PromptConfig(max_tokens=1000, reserved_tokens=100, strategy=strategy)
            prompt_manager = PromptManager(config)
