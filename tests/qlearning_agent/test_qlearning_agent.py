...
import numpy as np
from ece.agents.tier3.qlearning.qlearning_agent import QLearningGraphAgent, MemoryPath


@pytest.fixture
def mock_graph_manager():
    """Create a mock graph manager for testing"""
    mock_graph_manager.find_nodes_by_keywords = AsyncMock(
        return_value=[{"id": "start", "name": "start"}, {"id": "end", "name": "end"}]
    )
...
async def test_find_optimal_path(q_learning_agent):
    ...
    with patch.object(
        q_learning_agent, "_q_learning_pathfinding", new_callable=AsyncMock
    ) as mock_pathfinding:
        mock_path = MemoryPath(
            nodes=["start", "end"],
            relationships=[
                {"start_node": "start", "end_node": "end", "type": "RELATED_TO"}
            ],
            score=0.8,
            length=1,
        )
...
async def test_train(q_learning_agent):
    ...
    with patch.object(
        ...
        # Training data
        training_data = [("start", "end", 0.8), ("node1", "node2", 0.6)]
...
async def test_start_continuous_training(q_learning_agent):
    ...
    with patch.object(
        q_learning_agent, "_continuous_training_loop", new_callable=AsyncMock
    ) as mock_loop:
...
async def test_stop_continuous_training(q_learning_agent):
    ...
    with patch.object(
        ...
        assert (
            q_learning_agent.training_task is None
            or q_learning_agent.training_task.cancelled()
        )
...
def test_get_convergence_metrics(q_learning_agent):
    ...
    q_learning_agent.q_table = {
        "node1": {"node2:RELATED_TO": 0.5, "node3:CONNECTED_TO": 0.8},
        "node2": {"node4:LINKED_TO": 0.3},
    }
...
async def test_refine_relationships(q_learning_agent):
    ...
    path = MemoryPath(
        nodes=["nodeA", "nodeB"],
        relationships=[
            {"start_node": "nodeA", "end_node": "nodeB", "type": "RELATES_TO"}
        ],
        score=0.5,
        length=1,
    )
    ...
    with patch.object(
        q_learning_agent, "update_q_values", new_callable=AsyncMock
    ) as mock_update_q_values:
