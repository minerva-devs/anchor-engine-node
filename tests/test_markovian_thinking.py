...
class TestTRMClient(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_refine_thought(self):
        ...
        with patch.object(self.client.client, "post") as mock_post:
            ...
            mock_response.json.return_value = {
                "refined_thought": "This is a refined thought."
            }
            mock_response.raise_for_status = MagicMock()
            mock_post.return_value = mock_response

            result = await self.client.refine_thought(
                "test query", "original thought", "carryover"
            )
    ...
    async def test_iterative_refinement(self):
        ...
        with (
            patch.object(self.client, "critique_thought") as mock_critique,
            ...
            result = await self.client.iterative_refinement(
                "test query", "initial draft", max_iterations=3
            )
...
class TestMarkovianThinker(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_markovian_reasoning_loop(self):
        ...
        result = await self.thinker.markovian_reasoning_loop(
            "test query", "initial context"
        )

        self.assertEqual(result, "Final answer")
        self.thinker._get_initial_draft.assert_called_once_with(
            "initial context\n\ntest query"
        )
        self.thinker._iterative_refinement.assert_called_once_with(
            "test query", "Initial draft"
        )
        self.thinker._get_final_answer.assert_called_once_with(
            "test query", "Refined thought"
        )

    async def test_iterative_refinement(self):
        """Test the iterative refinement process."""
        # Set up the TRM client mock
        self.mock_trm_client.refine_thought = AsyncMock(
            side_effect=["First refinement", "Second refinement"]
        )
    ...
    async def test_markovian_chunked_reasoning(self):
        ...
        self.thinker._iterative_refinement = AsyncMock(
            return_value="final refined result"
        )
...
class TestOrchestratorMarkovianIntegration(unittest.IsolatedAsyncioTestCase):
    ...
    def setUp(self, mock_utcp_client, mock_cache_manager, mock_archivist_client):
        ...
        from ece.agents.tier1.orchestrator.orchestrator_agent import (
    ...
    async def test_should_use_markovian_thinking(self):
        ...
        simple_prompts = ["Hello", "What is the weather?", "Simple question"]
