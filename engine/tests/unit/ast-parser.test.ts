...
`;
  ...
  it('finds the class declaration', () => {
    const result = ok(parseCodeStructure(code, 'typescript'));
    const cls = result.blocks.find(b => b.type === 'class');
    expect(cls).toBeDefined();
    expect(cls?.name).toBe('AuthService');
  });

  it('finds methods inside the class', () => {
    const result = ok(parseCodeStructure(code, 'typescript'));
  ...
  it('captures imports from the file', () => {
    const result = ok(parseCodeStructure(code, 'typescript'));
  ...
  it('has correct start / end lines', () => {
    const result = ok(parseCodeStructure(code, 'typescript'));
  ...
  it('returns blocks sorted by byte order', () => {
    const result = ok(parseCodeStructure(code, 'typescript'));
...
`;
  ...
  it('finds class declaration', () => {
    const result = parseCodeStructure(code, 'javascript');
    ...
    if (result) {
      ...
      const tsResult = parseCodeStructure(code, 'typescript');
      expect(tsResult?.blocks.length).toBeGreaterThan(0);
    }
  });

  it('captures methods with class context', () => {
    const result = parseCodeStructure(code, 'javascript');
    if (result) {
...
describe('parseCodeStructure (edge cases)', () => {
  it('handles empty string', () => {
    const result = parseCodeStructure('', 'typescript');
    expect(result?.blocks.length).toBe(0);
  });

  it('handles code with no exports or named functions', () => {
  ...
  it('handles deeply nested class methods', () => {
    const code = `
export class Outer {
  innerMethod() {}
}
`;
  ...
  it('handles TSX (React component)', () => {
    const code = `
...
`;
    const result = ok(parseCodeStructure(code, 'tsx'));
    expect(result.blocks.length).toBeGreaterThan(0);
  });

  it('handles mixed file with prose comments', () => {
...
`;
  ...
  it('returns null when tree has errors', () => {
    // Unclosed braces should still parse in tree-sitter (it's forgiving)
    const result = ok(parseCodeStructure(`export class Foo`, 'typescript'));
    expect(result).toBeDefined();
  });
});
