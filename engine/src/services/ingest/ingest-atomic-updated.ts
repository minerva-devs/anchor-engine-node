...
export type { Compound } from '../../types/atomic.js';

export class AtomicIngestService {

    /**
     * Ingest result after compounds table removal.
     * 
     * @param compound - Minimal compound info (id, path, timestamp) for reference
     * @param molecules - Molecules to persist
     * @param atoms - Atoms to persist  
     * @param buckets - Target buckets for ingestion
     * @param provenance - Provenance type ('internal' | 'external')
     * @param molecularSignature - Molecular signature for compound-level deduplication
     */
    async ingestResult(
        compound: Compound,
        molecules: Molecule[],
        ...
        const filename = compound.path?.split(/[/\\]/).pop() || compound.id;

        console.log(`[AtomicIngest] ⏱️ START Persisting: ${filename} (${molecules.length} molecules, ${atoms.length} atoms)`);
        
        // Use provided provenance or derive from source
        const provenanceType = provenance || 'internal';
        const signature = molecularSignature || this.generateCompoundSignature(compound.id, compound.path);

        // Wrap entire ingestion in a transaction for atomicity and performance
        await db.transaction(async () => {
            await this._ingestResultInTransaction(
                compound, molecules, atoms, buckets, provenanceType, signature, onProgress
            );
        });
    ...
    private async _ingestResultInTransaction(
        ...
        const totalSteps = 5; // Removed compound persistence step
        let currentStep = 0;
        ...
        // 2. Persist Molecules with provenance and signature
        ...
        // LEGACY BRIDGE: Populate 'atoms' table (was 'memory')
        const memoryStart = Date.now();
        await this.batchWriteMemory(
            compound, molecules, atoms, buckets, molecularSignature, provenanceType
        );
        ...
        // 5. Persist Atom Positions (Lazy Molecule Inflation)
        ...
        for (let i = 0; i < atoms.length; i += chunkSize) {
            ...
            for (const atom of chunk) {
                await db.run(
                    ...
                    [
                        atom.id,
                        'atom_source', // source_path - atoms table uses this field
                        Date.now(), // timestamp
                        '0', // simhash (placeholder)
                        JSON.stringify(this.zeroVector()), // embedding
                        provenanceType, // provenance - use the passed parameter
                        ['atoms'], // buckets
                        [atom.label], // tags
                    ],
                );
    ...
    private async batchWriteTags(atoms: Atom[], buckets: string[]) {
        ...
        for (const atom of atoms) {
            ...
            const tags = filteredTags.length > 0 ? filteredTags : [atom.label];
    ...
    private async batchWriteMolecules(
        ...
        while (i < molecules.length) {
            ...
            while (i < molecules.length && batch.length < MAX_BATCH_ROWS) {
                const m = molecules[i];
                const rowBytes = (m.id?.length ?? 0) + 200;
            ...
            for (const m of batch) {
                ...
                values.push(
                    m.id,
                    m.content || '',
                    m.compoundId || '', // May be empty if compound table removed
                    m.sequence || 0,
                    m.start_byte || 0,
                    m.end_byte || 0,
                    m.type || 'prose',
                    ...
                    m.numeric_unit || null,
                    molecularSignature, // Use compound-level signature for all molecules in this batch
    ...
    /**
     * LEGACY BRIDGE: Populates 'atoms' table for backward compatibility.
     * This is a separate table that stores compound-level references.
     */
    private async batchWriteMemory(
        ...
        // Only write if compounds table doesn't exist (post-migration)
        // or if we want to maintain backward compatibility
        ...
        try {
            await db.run(
                ...
                [
                    ...
                    molecularSignature, // Use signature as simhash placeholder
                    JSON.stringify(this.zeroVector()),
                    provenanceType,
                    buckets,
                    atoms.map(a => a.label),
            ...
            // Table might not exist after migration - that's OK
            if (!error.message?.includes('does not exist')) {
                console.warn('[AtomicIngest] Memory table write failed:', error);
            }
        }
        ...
        for (let i = 0; i < molecules.length; i += batchSize) {
            ...
            for (const m of batch) {
                const rawTags = (m.atoms || []).map(id => atoms.find(a => a.id === id)?.label).filter(l => l !== undefined);
    ...
    private async flushAtomPositionsBatch(flatValues: any[]) {
        ...
        try {
            ...
            // Table might not exist - that's OK for optional feature
            if (!error.message?.includes('does not exist')) {
                console.warn('[AtomicIngest] Atom positions write failed:', error);
            }
        }
    }

    /**
     * Generate a compound signature from ID and path.
     * This replaces the molecular_signature field after migration.
     */
    private generateCompoundSignature(id: string, path?: string): string {
        const str = id + (path || '') + Date.now();
        let hash = 0;
